...
import { ChevronDown, ChevronUp, Search, Database, AlertCircle } from 'lucide-react';
...
export function EvidenceTable({ evidence }: EvidenceTableProps) {
  ...
  // Handle empty data
  const validEvidence = evidence && evidence.length > 0 ? evidence : [];
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-6 shadow-xl">
      ...
      <div className="mt-4 text-xs text-gray-500 text-center">
        Showing {sortedEvidence.length} of {validEvidence.length} evidence signals
        {searchTerm && ` (filtered from ${validEvidence.length})`}
      </div>
    </div>
  );
}